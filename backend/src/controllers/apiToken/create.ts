...
export async function createApiToken(req: Request, res: Response) {
    try {
        ...
        const { name, description, validityPeriod, accessScope, permissions } = req.body;

        if (!organizationId || !name || !validityPeriod || !accessScope) {
            return res.status(400).json({
                success: false,
                message: 'Organization ID, name, validity period, and access scope are required',
            });
        }

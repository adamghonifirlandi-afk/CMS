...
function demoResponse(config: { url?: string; method?: string; data?: unknown }) {
  ...
  if (method === "put" && /^projects\/[^/]+$/.test(url)) { const item = state.projects.find((entry) => url.endsWith(entry.id)); if (item) { item.name = String(body.name || item.name); item.description = String(body.description ?? item.description); } data = item; }
  if (method === "delete" && /^projects\/[^/]+$/.test(url)) state.projects = state.projects.filter((item) => !url.endsWith(item.id));
  if (method === "delete" && /^organizations\/[^/]+$/.test(url)) state.organizations = state.organizations.filter((item) => !url.endsWith(item.id));
  if (method === "delete" && /content-builder\/(single-pages|multiple-pages|components)\//.test(url)) state.models = state.models.filter((item) => !url.endsWith(item.id));
  if (method === "delete" && url.includes("media-assets/")) state.media = state.media.filter((item) => !url.endsWith(item.id));
  if (method === "post" && url.includes("media-assets")) { const item = { id: `asset_demo_${Date.now()}`, title: String(body.title || "Demo upload"), fileName: String(body.fileName || "demo-upload.jpg"), fileUrl: String(body.fileUrl || "https://images.unsplash.com/photo-1497366811353-6870744d04b2?w=900&q=80"), mimeType: "image/jpeg", size: 320000 }; state.media.push(item); data = item; }
  if (method === "post" && url === "collaborators") data = { id: `member_demo_${Date.now()}`, name: String(body.name || body.email || "New collaborator"), email: String(body.email || "collaborator@example.com"), role: String(body.role || "EDITOR"), status: "PENDING" };
  if (method === "delete" && url.includes("api-tokens/tokens/")) state.tokens = state.tokens.map((item) => url.endsWith(item.id) ? { ...item, status: "REVOKED" } : item);
